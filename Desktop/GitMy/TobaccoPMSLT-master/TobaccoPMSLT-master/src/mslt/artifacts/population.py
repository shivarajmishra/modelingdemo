...
def ProcessDataTable(df, year_start, max_age):
	...
	# Sort the rows.
	df = df.sort_values(by=['year', 'age', 'sex', 'strata']).reset_index(drop=True)
	return df
...
class Population:
	...
	def __init__(self, data_dir, year_start):
		...
		# Retain only the necessary columns.
		df_life = df_life[[
			'age', 'sex', 'strata', 'disability_rate', 'mortality_rate',
			'mortality_apc', 'expenditure_rate', 'expenditure_rate_death',
			'income', 'income_death',
		]]
	...
	def get_expenditure_rate(self):
		"""Return the expenditure rate for each stratum."""
		df = self._df_life[['age', 'sex', 'strata', 'expenditure_rate']]
		df = df.rename(columns={'expenditure_rate': 'value'})
		...
		df = df.sort_values(['year_start', 'age_start', 'sex', 'strata'])
		df = df.reset_index(drop=True)

		return df

	def get_expenditure_rate_death(self):
		"""Return the cost of death for each stratum."""
		df = self._df_life[['age', 'sex', 'strata', 'expenditure_rate_death']]
		df = df.rename(columns={'expenditure_rate_death': 'value'})
	...
	def get_income(self):
		"""Return the income for each stratum."""
		df = self._df_life[['age', 'sex', 'strata', 'income']]
		df = df.rename(columns={'income': 'value'})
	...
	def get_income_death(self):
		"""Return the income for each stratum."""
		df = self._df_life[['age', 'sex', 'strata', 'income_death']]
		df = df.rename(columns={'income_death': 'value'})
	...
	def get_acmr_apc(self):
		...
		df = self._df_life[['year', 'age', 'sex', 'strata', 'mortality_apc']]
		...
		df = pd.concat(tables).sort_values(['year', 'age', 'sex', 'strata'])
		df = df.reset_index(drop=True)
	...
	def get_mortality_rate(self):
		...
		df_acmr = self._df_life[['age', 'sex', 'strata', 'mortality_rate']]
		...
		df = pd.concat(tables).sort_values(['year_start', 'age_start', 'sex', 'strata'])
		df = df.reset_index(drop=True)
